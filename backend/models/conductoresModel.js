...
const createConductor = async (conductor) => {
  ...
  const placaUpper = placa ? placa.toUpperCase() : placa;
  const [result] = await pool.query(
    "INSERT INTO Conductores (cedula, nombreCompleto, celular, placa, tipoVehiculo, destinoFinal, tipoVisita) VALUES (?, ?, ?, ?, ?, ?, ?)",
    [cedula, toSentenceCase(nombreCompleto), celular, placaUpper, toSentenceCase(tipoVehiculo), toSentenceCase(destinoFinal), toSentenceCase(tipoVisita)]
  );
  return { idConductor: result.insertId, ...conductor, placa: placaUpper, nombreCompleto: toSentenceCase(nombreCompleto) };
};
...
const updateConductor = async (id, conductor) => {
  ...
  await pool.query(
    ...
    [cedula, toSentenceCase(nombreCompleto), celular, placaUpper, toSentenceCase(tipoVehiculo), toSentenceCase(destinoFinal), toSentenceCase(tipoVisita), id]
  );
};
